...
exports.weatherDaily = weatherDaily;
...
function normalizeCurrentWeather(data) {
    ...
    const temp = typeof data?.main?.temp === "number" ? data.main.temp : undefined;
    const feelsLike = typeof data?.main?.feels_like === "number"
        ? data.main.feels_like
        : undefined;
    const humidity = typeof data?.main?.humidity === "number" ? data.main.humidity : undefined;
    const tempMin = typeof data?.main?.temp_min === "number" ? data.main.temp_min : undefined;
    const tempMax = typeof data?.main?.temp_max === "number" ? data.main.temp_max : undefined;
    const windSpeed = typeof data?.wind?.speed === "number"
        ? Number(data.wind.speed.toFixed(2))
        : undefined;
    const windGust = typeof data?.wind?.gust === "number"
        ? Number(data.wind.gust.toFixed(2))
        : undefined;
    const tempC = kelvinToC(temp ?? null);
    const hasRain = (Array.isArray(data?.weather) ? data.weather : []).some((w) => typeof w?.main === "string" && w.main.toLowerCase().includes("rain"));
    return {
        weather: [
            {
                main,
...
async function weatherDaily(req, res) {
    try {
        ...
        const forecastList = Array.isArray(payload?.list) ? payload.list : [];
        ...
        relevantForecasts.forEach((item) => {
            ...
            if (temp != null) {
                if (tempMinK == null || temp < tempMinK)
                    tempMinK = temp;
                if (tempMaxK == null || temp > tempMaxK)
                    tempMaxK = temp;
            }
            if (humidity != null) {
        ...
        const primaryWeather = Array.isArray(firstForecast?.weather) && firstForecast.weather.length
            ? firstForecast.weather[0]
            : null;
        const main = typeof primaryWeather?.main === "string" && primaryWeather.main
            ? primaryWeather.main
            : "Weather";
        const description = typeof primaryWeather?.description === "string" &&
            primaryWeather.description
            ? primaryWeather.description
            : undefined;
        const avgHumidity = humidityCount > 0 ? Math.round(humiditySum / humidityCount) : undefined;
        ...
        const summary = main && tempMinC != null && tempMaxC != null
